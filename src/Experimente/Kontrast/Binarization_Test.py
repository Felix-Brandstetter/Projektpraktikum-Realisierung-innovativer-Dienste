...
for filename in os.listdir(input_folder):
    ...
    if any(filename.lower().endswith(ext) for ext in supported_extensions):
        ...
        methods = ["kapur", "otsu", "triangle", "adaptive_threshold"]
        for method in methods:
            ...
            evaluation_ocrdata.to_excel(
                os.path.join(
                    file_output_folder, f"evaluation_ocrdata_{method}_dpi.xlsx"
                )
            )
            ...
            ocrkit.create_searchable_pdf(
                ...
                out_filename=os.path.join(
                    file_output_folder, f"tiff_image_{method}_dpi.pdf"
                ),
